...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if (preg_match("/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/", $_POST['new-password'])) {
                if (preg_match("/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/", $_POST['con-password'])) {
                    if ($_POST['new-password'] == $_POST['con-password']) {
                        ...
                        header("Location: /KuppiMate/src/view/ex-dashboard.php?id=108");
                        exit();
                    }
                } else {
                    header("Location: /KuppiMate/src/view/ex-dashboard.php?id=107");
                    exit();
                }
            } else {
                header("Location: /KuppiMate/src/view/ex-dashboard.php?id=107");
    ...
    if ($dbpasw && password_verify($currentPassword,$dbpasw)) {
        ...
        if ($user->updateProfile(Dbconnector::getConnection(), "password-update", $currentPassword)) {
            header("Location: /KuppiMate/src/view/ex-dashboard.php?id=109");
            exit();
        } else {
            header("Location: /KuppiMate/src/view/ex-dashboard.php?id=111");
            exit();
        }
    } else {
        header("Location: /KuppiMate/src/view/ex-dashboard.php?id=110");
        exit();
    }
}
